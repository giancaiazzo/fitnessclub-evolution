...
export default function ClassesPage() {
  ...
  Object.values(fullSchedule).forEach((daySchedule) => {
    (daySchedule as ScheduleSlot[]).forEach((slot) => {
  ...
  const sortedTimes = Array.from(allTimes).sort((a, b) => {
    const parseTime = (time: string) => {
      ...
      if (time.includes("PM") && rawHour !== "12") {
        hour += 12;
      }

      if (time.includes("AM") && rawHour === "12") {
        hour = 0;
      }
  ...
  const findClassAtTime = (daySchedule: ScheduleSlot[], time: string) => {
              ...
              <tbody className="text-muted-foreground">
                {sortedTimes.map((time) => (
                  ...
                  >
                    ...
                    {days.map((day) => {
                      const daySchedule = fullSchedule[
                        day.key as keyof typeof fullSchedule
                      ] as ScheduleSlot[];

                      const classSlot = findClassAtTime(daySchedule, time);
                          ...
                          {classSlot ? (
                            <div>
                              <div className="font-medium text-foreground">
                                {classSlot.class ?? classSlot.className}
                              </div>
